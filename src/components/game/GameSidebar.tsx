                    ...
                    <div className="flex h-full flex-1 flex-col overflow-hidden">
                        ...
                        <div className="mt-4 flex flex-1 flex-col overflow-hidden px-6 pb-6">
                            ...
                            <div className="flex-1 space-y-2 overflow-y-auto pr-2 font-mono text-[10px]">
                                {gameState.log.map((entry, index) => <div key={`${index}-${entry}`} className={`border-l-2 py-1 pl-2 transition-all duration-300 ${index === 0 ? 'animate-pulse border-yellow-500 bg-white/5 text-white' : 'border-slate-800 text-slate-500'}`}>{entry}</div>)}
                            </div>
                        </div>
                    </div>
                ) : (
